...
export function urlFor(source: any, { width = 1200, height, quality = 70 }: SanityImageOptions = {}) {
...
export default function BlogPostCard({ post, isService = false }: BlogPostCardProps) {
  const imageUrl = post?.mainImage?.asset ? urlFor(post.mainImage) : null;
	console.log("TCL: BlogPostCard -> imageUrl", imageUrl)

  return (
    <Link
      href={`/${isService ? "service" : "blog"}/${post.slug.current}`}
      className="group relative block h-full animate-fadeIn"
    >
      ...
      <div className="relative bg-black/80 backdrop-blur-sm border border-white/5 rounded-lg overflow-hidden flex flex-col md:flex-row h-full transition-all duration-500 group-hover:shadow-[0_0_25px_rgba(230,199,139,0.2)]">
        ...
        <div className="p-6 relative flex flex-col flex-grow md:w-1/2 lg:w-3/5">
          ...
          <h2 className="text-2xl md:text-3xl font-playfair text-white group-hover:text-[#E6C78B] transition-colors duration-300 mb-3 leading-tight">
            {post.title}
          </h2>
